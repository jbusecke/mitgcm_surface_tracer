...
import numpy as np
import os
from mitgcm_surface_tracer.velocity_processing import validmask_aviso,combine_validmask
from mitgcm_surface_tracer.utils import writebin,readbin
from numpy.testing import assert_allclose
...
def test_validmask_aviso():
    ...
    mask = validmask_aviso(a,a)
    test = np.any(np.isnan(a),axis = a.get_axis_num('time'))
>>>>>>> started refactoring velocity processing and added tests
...
=======
    ...
    test_file = file = tmpdir.join('validmask_combined.bin')
    assert_allclose(combo,readbin(test_file.strpath,a.shape))
>>>>>>> started refactoring velocity processing and added tests
