...
def validmask_aviso(da_u,da_v):
    validmask_aviso_u = np.any(np.isnan(da_u),axis = da_u.get_axis_num('time'))
    validmask_aviso_v = np.any(np.isnan(da_v),axis = da_v.get_axis_num('time'))

    if np.logical_xor(validmask_aviso_u,validmask_aviso_v).any():
        raise RuntimeWarning('U and V validmask are not equal')

    validmask_aviso = np.logical_or(validmask_aviso_u,validmask_aviso_v)
...
def combine_validmask(data_dir,shape=None,debug=False):
    ...
    combo = np.any(np.stack(masks,axis=2),axis=2)

    fpath = data_dir+'/validmask_combined.bin'
    writebin(combo,fpath)
    print('--- combined validmask written to '+fpath+' ---')

def process_aviso(odir,gdir,ddir_dt,fid_dt,ddir_nrt=None,fid_nrt=None,interpolate=True):
    ...
    validmask_mit = interp_map_regular_grid(validmask,ds.u.lon.data,ds.u.lat.data,grid.XC.data,grid.YC.data)
    ...
    # for tt in [0]:
        ...
        u_int = interp_map_regular_grid(u,ds.u.lon.data,ds.u.lat.data,grid.XG.data,grid.YC.data)
        v_int = interp_map_regular_grid(v,ds.v.lon.data,ds.v.lat.data,grid.XC.data,grid.YG.data)
    ...
    print("--- Velocity Interpolation took %s seconds ---" % (time.time() - start_time))
